...
var Tag = class _Tag {
  ...
  @internal
  */
  constructor(name3, set, base2, modified) {
    this.name = name3;
    this.set = set;
    this.base = base2;
...
// node_modules/@codemirror/lang-javascript/node_modules/@codemirror/state/dist/index.js
...
var DefaultSplit = /\r\n?|\n/;
var MapMode = function(MapMode3) {
  MapMode3[MapMode3["Simple"] = 0] = "Simple";
  MapMode3[MapMode3["TrackDel"] = 1] = "TrackDel";
  MapMode3[MapMode3["TrackBefore"] = 2] = "TrackBefore";
  MapMode3[MapMode3["TrackAfter"] = 3] = "TrackAfter";
  return MapMode3;
}(MapMode || (MapMode = {}));
...
function flatten(extension, compartments, newCompartments) {
  ...
  function inner(ext, prec3) {
    ...
    if (Array.isArray(ext)) {
      ...
      inner(content3, prec3);
    } else if (ext instanceof PrecExtension) {
      inner(ext.inner, ext.prec);
    } else if (ext instanceof StateField) {
      result[prec3].push(ext);
      if (ext.provides)
        inner(ext.provides, prec3);
...
var CharCategory = function(CharCategory3) {
...
// node_modules/@codemirror/view/node_modules/@codemirror/state/dist/index.js
var Text2 = class _Text {
  ...
  */
  replace(from, to, text) {
    ...
    return TextNode2.from(parts, this.length - (to - from) + text.length);
  }
  /**
  Append another document to this one.
  */
  ...
  */
  slice(from, to = this.length) {
    ...
    let parts = [];
    this.decompose(from, to, parts, 0);
    return TextNode2.from(parts, to - from);
  }
  /**
  Test whether this text is equal to another instance.
  */
  eq(other) {
    ...
    if (other.length != this.length || other.lines != this.lines)
      return false;
    let start = this.scanIdentical(other, 1), end = this.length - this.scanIdentical(other, -1);
    let a = new RawTextCursor2(this), b = new RawTextCursor2(other);
    for (let skip = start, pos = start; ; ) {
      a.next(skip);
      b.next(skip);
      skip = 0;
  ...
  */
  iter(dir = 1) {
    return new RawTextCursor2(this, dir);
  }
  /**
  Iterate over a range of the text. When `from` > `to`, the
  iterator will run in reverse.
  */
  iterRange(from, to = this.length) {
    return new PartialTextCursor2(this, from, to);
  }
  /**
  Return a cursor that iterates over the given range of lines,
  _without_ returning the line breaks between, and yielding empty
  ...
  */
  static of(text) {
    ...
    if (text.length == 1 && !text[0])
      return _Text.empty;
    return text.length <= 32 ? new TextLeaf2(text) : TextNode2.from(TextLeaf2.split(text, []));
  }
};
var TextLeaf2 = class _TextLeaf extends Text2 {
  constructor(text, length = textLength2(text)) {
    super();
    this.text = text;
    this.length = length;
  }
  ...
  lineInner(target, isLine, line, offset) {
    for (let i = 0; ; i++) {
      let string3 = this.text[i], end = offset + string3.length;
      if ((isLine ? line : end) >= target)
        return new Line2(offset, end, line, string3);
      offset = end + 1;
      line++;
    }
  }
  decompose(from, to, target, open) {
    let text = from <= 0 && to >= this.length ? this : new _TextLeaf(sliceText2(this.text, from, to), Math.min(to, this.length) - Math.max(0, from));
    if (open & 1) {
      let prev = target.pop();
      let joined = appendText2(text.text, prev.text.slice(), 0, text.length);
      if (joined.length <= 32) {
        target.push(new _TextLeaf(joined, prev.length + text.length));
      } else {
        let mid = joined.length >> 1;
  ...
  replace(from, to, text) {
    if (!(text instanceof _TextLeaf))
      return super.replace(from, to, text);
    [from, to] = clip2(this, from, to);
    let lines = appendText2(this.text, appendText2(text.text, sliceText2(this.text, 0, from)), to);
    let newLen = this.length + text.length - (to - from);
    if (lines.length <= 32)
      return new _TextLeaf(lines, newLen);
    return TextNode2.from(_TextLeaf.split(lines, []), newLen);
  }
  sliceString(from, to = this.length, lineSep = "\n") {
    [from, to] = clip2(this, from, to);
    let result = "";
...
var TextNode2 = class _TextNode extends Text2 {
  constructor(children, length) {
    super();
    this.children = children;
    this.length = length;
  ...
  static from(children, length = children.reduce((l, ch) => l + ch.length + 1, -1)) {
    ...
    if (lines < 32) {
      ...
      for (let ch of children)
        ch.flatten(flat);
      return new TextLeaf2(flat, length);
    }
    let chunk = Math.max(
      32,
      lines >> 5
    ...
    function add(child) {
      ...
      if (child.lines > maxChunk && child instanceof _TextNode) {
        ...
        flush();
        chunked.push(child);
      } else if (child instanceof TextLeaf2 && currentLines && (last = currentChunk[currentChunk.length - 1]) instanceof TextLeaf2 && child.lines + last.lines <= 32) {
        currentLines += child.lines;
        currentLen += child.length + 1;
        currentChunk[currentChunk.length - 1] = new TextLeaf2(last.text.concat(child.text), last.length + 1 + child.length);
      } else {
        if (currentLines + child.lines > chunk)
          flush();
        currentLines += child.lines;
...
Text2.empty = new TextLeaf2([""], 0);
function textLength2(text) {
  let length = -1;
  for (let line of text)
    length += line.length + 1;
  return length;
}
function appendText2(text, target, from = 0, to = 1e9) {
  for (let pos = 0, i = 0, first = true; i < text.length && pos <= to; i++) {
    let line = text[i], end = pos + line.length;
    if (end >= from) {
      if (end > to)
...
function sliceText2(text, from, to) {
  return appendText2(text, [""], from, to);
}
var RawTextCursor2 = class {
  constructor(text, dir = 1) {
    this.dir = dir;
    this.done = false;
    this.lineBreak = false;
    this.value = "";
    this.nodes = [text];
    this.offsets = [dir > 0 ? 1 : (text instanceof TextLeaf2 ? text.text.length : text.children.length) << 1];
  }
  nextInner(skip, dir) {
    this.done = this.lineBreak = false;
    for (; ; ) {
      let last = this.nodes.length - 1;
      let top = this.nodes[last], offsetValue = this.offsets[last], offset = offsetValue >> 1;
      let size = top instanceof TextLeaf2 ? top.text.length : top.children.length;
      if (offset == (dir > 0 ? size : 0)) {
        if (last == 0) {
          this.done = true;
          this.value = "";
        ...
        skip--;
      } else if (top instanceof TextLeaf2) {
        let next = top.text[offset + (dir < 0 ? -1 : 0)];
        this.offsets[last] += dir;
        if (next.length > Math.max(0, skip)) {
          this.value = skip == 0 ? next : dir > 0 ? next.slice(skip) : next.slice(0, next.length - skip);
        ...
        if (skip > next.length) {
          ...
          if (dir < 0)
            this.offsets[last]--;
          this.nodes.push(next);
          this.offsets.push(dir > 0 ? 1 : (next instanceof TextLeaf2 ? next.text.length : next.children.length) << 1);
        }
      }
    }
  }
...
var PartialTextCursor2 = class {
  constructor(text, start, end) {
    this.value = "";
    this.done = false;
    this.cursor = new RawTextCursor2(text, start > end ? -1 : 1);
    this.pos = start > end ? text.length : 0;
    this.from = Math.min(start, end);
    this.to = Math.max(start, end);
  }
...
if (typeof Symbol != "undefined") {
  ...
  RawTextCursor2.prototype[Symbol.iterator] = PartialTextCursor2.prototype[Symbol.iterator] = LineCursor2.prototype[Symbol.iterator] = function() {
    return this;
  };
}
var Line2 = class {
...
function clip2(text, from, to) {
  from = Math.max(0, Math.min(text.length, from));
  return [from, Math.max(from, Math.min(text.length, to))];
}
var extend2 = "lc,34,7n,7,7b,19,,,,2,,2,,,20,b,1c,l,g,,2t,7,2,6,2,2,,4,z,,u,r,2j,b,1m,9,9,,o,4,,9,,3,,5,17,3,3b,f,,w,1j,,,,4,8,4,,3,7,a,2,t,,1m,,,,2,4,8,,9,,a,2,q,,2,2,1l,,4,2,4,2,2,3,3,,u,2,3,,b,2,1l,,4,5,,2,4,,k,2,m,6,,,1m,,,2,,4,8,,7,3,a,2,u,,1n,,,,c,,9,,14,,3,,1l,3,5,3,,4,7,2,b,2,t,,1m,,2,,2,,3,,5,2,7,2,b,2,s,2,1l,2,,,2,4,8,,9,,a,2,t,,20,,4,,2,3,,,8,,29,,2,7,c,8,2q,,2,9,b,6,22,2,r,,,,,,1j,e,,5,,2,5,b,,10,9,,2u,4,,6,,2,2,2,p,2,4,3,g,4,d,,2,2,6,,f,,jj,3,qa,3,t,3,t,2,u,2,1s,2,,7,8,,2,b,9,,19,3,3b,2,y,,3a,3,4,2,9,,6,3,63,2,2,,1m,,,7,,,,,2,8,6,a,2,,1c,h,1r,4,1c,7,,,5,,14,9,c,2,w,4,2,2,,3,1k,,,2,3,,,3,1m,8,2,2,48,3,,d,,7,4,,6,,3,2,5i,1m,,5,ek,,5f,x,2da,3,3x,,2o,w,fe,6,2x,2,n9w,4,,a,w,2,28,2,7k,,3,,4,,p,2,5,,47,2,q,i,d,,12,8,p,b,1a,3,1c,,2,4,2,2,13,,1v,6,2,2,2,2,c,,8,,1b,,1f,,,3,2,2,5,2,,,16,2,8,,6m,,2,,4,,fn4,,kh,g,g,g,a6,2,gt,,6a,,45,5,1ae,3,,2,5,4,14,3,4,,4l,2,fx,4,ar,2,49,b,4w,,1i,f,1k,3,1d,4,2,2,1x,3,10,5,,8,1q,,c,2,1g,9,a,4,2,,2n,3,2,,,2,6,,4g,,3,8,l,2,1l,2,,,,,m,,e,7,3,5,5f,8,2,3,,,n,,29,,2,6,,,2,,,2,,2,6j,,2,4,6,2,,2,r,2,2d,8,2,,,2,2y,,,,2,6,,,2t,3,2,4,,5,77,9,,2,6t,,a,2,,,4,,40,4,2,2,4,,w,a,14,6,2,4,8,,9,6,2,3,1a,d,,2,ba,7,,6,,,2a,m,2,7,,2,,2,3e,6,3,,,2,,7,,,20,2,3,,,,9n,2,f0b,5,1n,7,t4,,1r,4,29,,f5k,2,43q,,,3,4,5,8,8,2,7,u,4,44,3,1iz,1j,4,1e,8,,e,,m,5,,f,11s,7,,h,2,7,,2,,5,79,7,c5,4,15s,7,31,7,240,5,gx7k,2o,3k,6o".split(",").map((s) => s ? parseInt(s, 36) : 1);
for (let i = 1; i < extend2.length; i++)
  extend2[i] += extend2[i - 1];
function isExtendingChar2(code) {
  for (let i = 1; i < extend2.length; i += 2)
    if (extend2[i] > code)
      return extend2[i - 1] <= code;
  return false;
}
function isRegionalIndicator2(code) {
  return code >= 127462 && code <= 127487;
}
var ZWJ2 = 8205;
function findClusterBreak2(str, pos, forward = true, includeExtending = true) {
  return (forward ? nextClusterBreak2 : prevClusterBreak2)(str, pos, includeExtending);
}
function nextClusterBreak2(str, pos, includeExtending) {
  if (pos == str.length)
    return pos;
  if (pos && surrogateLow2(str.charCodeAt(pos)) && surrogateHigh2(str.charCodeAt(pos - 1)))
    pos--;
  let prev = codePointAt2(str, pos);
  pos += codePointSize2(prev);
  while (pos < str.length) {
    let next = codePointAt2(str, pos);
    if (prev == ZWJ2 || next == ZWJ2 || includeExtending && isExtendingChar2(next)) {
      pos += codePointSize2(next);
      prev = next;
    } else if (isRegionalIndicator2(next)) {
      let countBefore = 0, i = pos - 2;
      while (i >= 0 && isRegionalIndicator2(codePointAt2(str, i))) {
        countBefore++;
        i -= 2;
      }
      if (countBefore % 2 == 0)
...
function prevClusterBreak2(str, pos, includeExtending) {
  while (pos > 0) {
    let found = nextClusterBreak2(str, pos - 2, includeExtending);
    if (found < pos)
      return found;
    pos--;
  }
...
var DefaultSplit2 = /\r\n?|\n/;
var MapMode2 = function(MapMode3) {
  MapMode3[MapMode3["Simple"] = 0] = "Simple";
  MapMode3[MapMode3["TrackDel"] = 1] = "TrackDel";
  MapMode3[MapMode3["TrackBefore"] = 2] = "TrackBefore";
  MapMode3[MapMode3["TrackAfter"] = 3] = "TrackAfter";
...
var ChangeDesc2 = class _ChangeDesc {
  // Sections are encoded as pairs of integers. The first is the
  // length in the current document, and the second is -1 for
  // unaffected sections, and the length of the replacement content
  // otherwise. So an insertion would be (0, n>0), a deletion (n>0,
  ...
  */
  iterChangedRanges(f, individual = false) {
    iterChanges2(this, f, individual);
  }
  /**
  Get a description of the inverted form of these changes.
  */
  ...
  */
  composeDesc(other) {
    return this.empty ? other : other.empty ? this : composeSets2(this, other);
  }
  /**
  Map this description, which should start with the same document
  as `other`, over another set of changes, so that it can be
  ...
  */
  mapDesc(other, before = false) {
    return other.empty ? this : mapSet2(this, other, before);
  }
  mapPos(pos, assoc = -1, mode = MapMode2.Simple) {
    let posA = 0, posB = 0;
    for (let i = 0; i < this.sections.length; ) {
      let len = this.sections[i++], ins = this.sections[i++], endA = posA + len;
      if (ins < 0) {
        ...
        if (mode != MapMode2.Simple && endA >= pos && (mode == MapMode2.TrackDel && posA < pos && endA > pos || mode == MapMode2.TrackBefore && posA < pos || mode == MapMode2.TrackAfter && endA > pos))
          return null;
        if (endA > pos || endA == pos && assoc < 0 && !len)
          return pos == posA || assoc < 0 ? posB : posB + ins;
        posB += ins;
...
var ChangeSet2 = class _ChangeSet extends ChangeDesc2 {
  constructor(sections, inserted) {
    super(sections);
    this.inserted = inserted;
  }
  ...
  */
  apply(doc2) {
    if (this.length != doc2.length)
      throw new RangeError("Applying change set to a document with the wrong length");
    iterChanges2(this, (fromA, toA, fromB, _toB, text) => doc2 = doc2.replace(fromB, fromB + (toA - fromA), text), false);
    return doc2;
  }
  mapDesc(other, before = false) {
    return mapSet2(this, other, before, true);
  }
  /**
  Given the document as it existed _before_ the changes, return a
  change set that represents the inverse of this set, which could
  ...
  */
  compose(other) {
    return this.empty ? other : other.empty ? this : composeSets2(this, other, true);
  }
  /**
  Given another change set starting in the same document, maps this
  change set over the other, producing a new change set that can be
  ...
  */
  map(other, before = false) {
    return other.empty ? this : mapSet2(this, other, before, true);
  }
  /**
  Iterate over the changed ranges in the document, calling `f` for
  each, with the range in the original document (`fromA`-`toA`)
  ...
  */
  get desc() {
    return ChangeDesc2.create(this.sections);
  }
  /**
  @internal
  */
    ...
    let iter = new SectionIter2(this);
      ...
      while (pos < next || pos == next && iter.len == 0) {
        if (iter.done)
          break done;
        let len = Math.min(iter.len, next - pos);
        addSection2(filteredSections, len, -1);
        let ins = iter.ins == -1 ? -1 : iter.off == 0 ? iter.ins : 0;
        addSection2(resultSections, len, ins);
        if (ins > 0)
      ...
      while (pos < end) {
        if (iter.done)
          break done;
        let len = Math.min(iter.len, end - pos);
        addSection2(resultSections, len, -1);
        addSection2(filteredSections, len, iter.ins == -1 ? -1 : iter.off == 0 ? iter.ins : 0);
        iter.forward(len);
        pos += len;
      }
    }
  ...
  static of(changes, length, lineSep) {
    ...
    function flush(force = false) {
      if (!force && !sections.length)
        return;
      if (pos < length)
        addSection2(sections, length - pos, -1);
      let set = new _ChangeSet(sections, inserted);
      total = total ? total.compose(set.map(total)) : set;
      sections = [];
      inserted = [];
    ...
    function process2(spec) {
      if (Array.isArray(spec)) {
        ...
        if (from > to || from < 0 || to > length)
          throw new RangeError(`Invalid change range ${from} to ${to} (in doc of length ${length})`);
        let insText = !insert3 ? Text2.empty : typeof insert3 == "string" ? Text2.of(insert3.split(lineSep || DefaultSplit2)) : insert3;
        let insLen = insText.length;
        if (from == to && insLen == 0)
          return;
        if (from < pos)
          flush();
        if (from > pos)
          addSection2(sections, from - pos, -1);
        addSection2(sections, to - from, insLen);
...
function addSection2(sections, len, ins, forceJoin = false) {
  if (len == 0 && ins <= 0)
    return;
  let last = sections.length - 2;
  if (last >= 0 && ins <= 0 && ins == sections[last + 1])
...
function iterChanges2(desc, f, individual) {
  let inserted = desc.inserted;
  for (let posA = 0, posB = 0, i = 0; i < desc.sections.length; ) {
    let len = desc.sections[i++], ins = desc.sections[i++];
    if (ins < 0) {
...
function mapSet2(setA, setB, before, mkSet = false) {
  let sections = [], insert3 = mkSet ? [] : null;
  let a = new SectionIter2(setA), b = new SectionIter2(setB);
  for (let inserted = -1; ; ) {
    if (a.ins == -1 && b.ins == -1) {
      let len = Math.min(a.len, b.len);
      addSection2(sections, len, -1);
      a.forward(len);
      b.forward(len);
    } else if (b.ins >= 0 && (a.ins < 0 || inserted == a.i || a.off == 0 && (b.len < a.len || b.len == a.len && !before))) {
      let len = b.len;
      addSection2(sections, b.ins, -1);
      while (len) {
        let piece = Math.min(a.len, len);
        if (a.ins >= 0 && inserted < a.i && a.len <= piece) {
          addSection2(sections, 0, a.ins);
          if (insert3)
            addInsert2(insert3, sections, a.text);
          inserted = a.i;
        }
        a.forward(piece);
        len -= piece;
      ...
      addSection2(sections, len, inserted < a.i ? a.ins : 0);
      if (insert3 && inserted < a.i)
        addInsert2(insert3, sections, a.text);
      inserted = a.i;
      a.forward(a.len - left);
    } else if (a.done && b.done) {
      return insert3 ? ChangeSet2.createSet(sections, insert3) : ChangeDesc2.create(sections);
    } else {
      throw new Error("Mismatched change set lengths");
    }
  }
}
function composeSets2(setA, setB, mkSet = false) {
  let sections = [];
  let insert3 = mkSet ? [] : null;
  let a = new SectionIter2(setA), b = new SectionIter2(setB);
  for (let open = false; ; ) {
    if (a.done && b.done) {
      ...
      addSection2(sections, a.len, 0, open);
      a.next();
    } else if (b.len == 0 && !b.done) {
      addSection2(sections, 0, b.ins, open);
      if (insert3)
        addInsert2(insert3, sections, b.text);
      b.next();
    } else if (a.done || b.done) {
      throw new Error("Mismatched change set lengths");
    } else {
      ...
      if (a.ins == -1) {
        let insB = b.ins == -1 ? -1 : b.off ? 0 : b.ins;
        addSection2(sections, len, insB, open);
        if (insert3 && insB)
          addInsert2(insert3, sections, b.text);
      } else if (b.ins == -1) {
        addSection2(sections, a.off ? 0 : a.len, len, open);
        if (insert3)
          addInsert2(insert3, sections, a.textBit(len));
      } else {
        addSection2(sections, a.off ? 0 : a.len, b.off ? 0 : b.ins, open);
        if (insert3 && !b.off)
          addInsert2(insert3, sections, b.text);
      }
      open = (a.ins > len || b.ins >= 0 && b.len > len) && (open || sections.length > sectionLen);
...
var SelectionRange2 = class _SelectionRange {
  constructor(from, to, flags) {
    this.from = from;
    this.to = to;
    this.flags = flags;
  ...
  */
  extend(from, to = from) {
    if (from <= this.anchor && to >= this.anchor)
      return EditorSelection2.range(from, to);
    let head = Math.abs(from - this.anchor) > Math.abs(to - this.anchor) ? from : to;
    return EditorSelection2.range(this.anchor, head);
  }
  /**
  Compare this range to another range.
  */
  ...
  */
  static fromJSON(json) {
    if (!json || typeof json.anchor != "number" || typeof json.head != "number")
      throw new RangeError("Invalid JSON representation for SelectionRange");
    return EditorSelection2.range(json.anchor, json.head);
  }
  /**
  @internal
  */
...
var EditorSelection2 = class _EditorSelection {
  constructor(ranges, mainIndex) {
    this.ranges = ranges;
    this.mainIndex = mainIndex;
  }
  ...
  */
  static fromJSON(json) {
    if (!json || !Array.isArray(json.ranges) || typeof json.main != "number" || json.main >= json.ranges.length)
      throw new RangeError("Invalid JSON representation for EditorSelection");
    return new _EditorSelection(json.ranges.map((r) => SelectionRange2.fromJSON(r)), json.main);
  }
  /**
  Create a selection holding a single range.
  */
  ...
  */
  static cursor(pos, assoc = 0, bidiLevel, goalColumn) {
    return SelectionRange2.create(pos, pos, (assoc == 0 ? 0 : assoc < 0 ? 8 : 16) | (bidiLevel == null ? 7 : Math.min(6, bidiLevel)) | (goalColumn !== null && goalColumn !== void 0 ? goalColumn : 16777215) << 6);
  }
  /**
  Create a selection range.
  */
  static range(anchor, head, goalColumn, bidiLevel) {
    let flags = (goalColumn !== null && goalColumn !== void 0 ? goalColumn : 16777215) << 6 | (bidiLevel == null ? 7 : Math.min(6, bidiLevel));
    return head < anchor ? SelectionRange2.create(head, anchor, 32 | 16 | flags) : SelectionRange2.create(anchor, head, (head > anchor ? 8 : 0) | flags);
  }
  /**
  @internal
  */
...
function checkSelection2(selection, docLength) {
  for (let range of selection.ranges)
    if (range.to > docLength)
      throw new RangeError("Selection points outside of document");
}
var nextID2 = 0;
var Facet2 = class _Facet {
  constructor(combine, compareInput, compare3, isStatic, enables) {
    this.combine = combine;
    this.compareInput = compareInput;
    this.compare = compare3;
  ...
  */
  static define(config2 = {}) {
    return new _Facet(config2.combine || ((a) => a), config2.compareInput || ((a, b) => a === b), config2.compare || (!config2.combine ? sameArray3 : (a, b) => a === b), !!config2.static, config2.enables);
  }
  /**
  Returns an extension that adds the given value to this facet.
  */
  of(value) {
    return new FacetProvider2([], this, 0, value);
  }
  /**
  Create an extension that computes a value for the facet from a
  state. You must take care to declare the parts of the state that
  ...
  */
  compute(deps, get) {
    if (this.isStatic)
      throw new Error("Can't compute a static facet");
    return new FacetProvider2(deps, this, 1, get);
  }
  /**
  Create an extension that computes zero or more values for this
  facet from a state.
  */
  computeN(deps, get) {
    if (this.isStatic)
      throw new Error("Can't compute a static facet");
    return new FacetProvider2(deps, this, 2, get);
  }
  from(field, get) {
    if (!get)
      get = (x) => x;
...
var FacetProvider2 = class {
  ...
  dynamicSlot(addresses) {
    ...
    return {
      ...
      update(state, tr) {
        if (depDoc && tr.docChanged || depSel && (tr.docChanged || tr.selection) || ensureAll2(state, depAddrs)) {
          let newVal = getter(state);
          if (multi ? !compareArray2(newVal, state.values[idx], compare3) : !compare3(newVal, state.values[idx])) {
            state.values[idx] = newVal;
            return 1;
          }
        }
      ...
      reconfigure: (state, oldState) => {
        let newVal, oldAddr = oldState.config.address[id2];
        if (oldAddr != null) {
          let oldVal = getAddr2(oldState, oldAddr);
          if (this.dependencies.every((dep) => {
            return dep instanceof Facet2 ? oldState.facet(dep) === state.facet(dep) : dep instanceof StateField2 ? oldState.field(dep, false) == state.field(dep, false) : true;
          }) || (multi ? compareArray2(newVal = getter(state), oldVal, compare3) : compare3(newVal = getter(state), oldVal))) {
            state.values[idx] = oldVal;
            return 0;
          }
        } else {
...
function compareArray2(a, b, compare3) {
  if (a.length != b.length)
    return false;
  for (let i = 0; i < a.length; i++)
    if (!compare3(a[i], b[i]))
...
function ensureAll2(state, addrs) {
  let changed = false;
  for (let addr of addrs)
    if (ensureAddr2(state, addr) & 1)
      changed = true;
  return changed;
}
function dynamicFacetSlot2(addresses, facet, providers) {
  let providerAddrs = providers.map((p) => addresses[p.id]);
  let providerTypes = providers.map((p) => p.type);
  let dynamic = providerAddrs.filter((p) => !(p & 1));
  let idx = addresses[facet.id] >> 1;
  function get(state) {
    let values = [];
    for (let i = 0; i < providerAddrs.length; i++) {
      let value = getAddr2(state, providerAddrs[i]);
      if (providerTypes[i] == 2)
        for (let val of value)
          values.push(val);
      else
  ...
  return {
    create(state) {
      for (let addr of providerAddrs)
        ensureAddr2(state, addr);
      state.values[idx] = get(state);
      return 1;
    },
    update(state, tr) {
      if (!ensureAll2(state, dynamic))
        return 0;
      let value = get(state);
      if (facet.compare(value, state.values[idx]))
        return 0;
    ...
    reconfigure(state, oldState) {
      let depChanged = ensureAll2(state, providerAddrs);
      let oldProviders = oldState.config.facets[facet.id], oldValue = oldState.facet(facet);
      if (oldProviders && !depChanged && sameArray3(providers, oldProviders)) {
        state.values[idx] = oldValue;
        return 0;
      }
      let value = get(state);
...
var initField2 = Facet2.define({ static: true });
var StateField2 = class _StateField {
  ...
  */
  static define(config2) {
    let field = new _StateField(nextID2++, config2.create, config2.update, config2.compare || ((a, b) => a === b), config2);
    if (config2.provide)
      field.provides = config2.provide(field);
    return field;
  }
  create(state) {
    let init = state.facet(initField2).find((i) => i.field == this);
    return ((init === null || init === void 0 ? void 0 : init.create) || this.createF)(state);
  }
  /**
  @internal
  ...
  */
  init(create) {
    return [this, initField2.of({ field: this, create })];
  }
  /**
  State field instances can be used as
  [`Extension`](https://codemirror.net/6/docs/ref/#state.Extension) values to enable the field in a
...
var Prec_2 = { lowest: 4, low: 3, default: 2, high: 1, highest: 0 };
function prec2(value) {
  return (ext) => new PrecExtension2(ext, value);
}
var Prec2 = {
  /**
  The highest precedence level, for extensions that should end up
  near the start of the precedence ordering.
  */
  highest: prec2(Prec_2.highest),
  /**
  A higher-than-default precedence, for extensions that should
  come before those with default precedence.
  */
  high: prec2(Prec_2.high),
  /**
  The default precedence, which is also used for extensions
  without an explicit precedence.
  */
  default: prec2(Prec_2.default),
  /**
  A lower-than-default precedence.
  */
  low: prec2(Prec_2.low),
  /**
  The lowest precedence level. Meant for things that should end up
  near the end of the extension order.
  */
  lowest: prec2(Prec_2.lowest)
};
var PrecExtension2 = class {
  constructor(inner, prec3) {
    this.inner = inner;
    this.prec = prec3;
  }
};
var Compartment2 = class _Compartment {
  /**
  Create an instance of this compartment to add to your [state
  configuration](https://codemirror.net/6/docs/ref/#state.EditorStateConfig.extensions).
  */
  of(ext) {
    return new CompartmentInstance2(this, ext);
  }
  /**
  Create an [effect](https://codemirror.net/6/docs/ref/#state.TransactionSpec.effects) that
  reconfigures this compartment.
...
var CompartmentInstance2 = class {
  constructor(compartment, inner) {
    this.compartment = compartment;
    this.inner = inner;
  }
  ...
  static resolve(base2, compartments, oldState) {
    ...
    for (let id2 in facets) {
      ...
      if (providers.every(
        ...
        address[facet.id] = staticValues.length << 1 | 1;
        if (sameArray3(oldProviders, providers)) {
          staticValues.push(oldState.facet(facet));
        } else {
          let value = facet.combine(providers.map((p) => p.value));
          staticValues.push(oldState && facet.compare(value, oldState.facet(facet)) ? oldState.facet(facet) : value);
        ...
        address[facet.id] = dynamicSlots.length << 1;
        dynamicSlots.push((a) => dynamicFacetSlot2(a, facet, providers));
      }
    }
    let dynamic = dynamicSlots.map((f) => f(address));
    return new _Configuration(base2, newCompartments, dynamic, address, staticValues, facets);
...
function flatten2(extension, compartments, newCompartments) {
  ...
  function inner(ext, prec3) {
    ...
    if (known != null) {
      ...
      if (found > -1)
        result[known].splice(found, 1);
      if (ext instanceof CompartmentInstance2)
        newCompartments.delete(ext.compartment);
    }
    seen.set(ext, prec3);
    if (Array.isArray(ext)) {
      for (let e of ext)
        inner(e, prec3);
    } else if (ext instanceof CompartmentInstance2) {
      if (newCompartments.has(ext.compartment))
        throw new RangeError(`Duplicate use of compartment in extensions`);
      let content3 = compartments.get(ext.compartment) || ext.inner;
      newCompartments.set(ext.compartment, content3);
      inner(content3, prec3);
    } else if (ext instanceof PrecExtension2) {
      inner(ext.inner, ext.prec);
    } else if (ext instanceof StateField2) {
      result[prec3].push(ext);
      if (ext.provides)
        inner(ext.provides, prec3);
    } else if (ext instanceof FacetProvider2) {
      result[prec3].push(ext);
      if (ext.facet.extensions)
        inner(ext.facet.extensions, Prec_2.default);
    } else {
      let content3 = ext.extension;
      if (!content3)
        throw new Error(`Unrecognized extension value in extension set (${ext}). This sometimes happens because multiple instances of @codemirror/state are loaded, breaking instanceof checks.`);
  ...
  inner(extension, Prec_2.default);
  return result.reduce((a, b) => a.concat(b));
}
function ensureAddr2(state, addr) {
  if (addr & 1)
    return 2;
  let idx = addr >> 1;
  let status = state.status[idx];
...
function getAddr2(state, addr) {
  return addr & 1 ? state.config.staticValues[addr >> 1] : state.values[addr >> 1];
}
var languageData2 = Facet2.define();
var allowMultipleSelections2 = Facet2.define({
  combine: (values) => values.some((v) => v),
  static: true
});
var lineSeparator2 = Facet2.define({
  combine: (values) => values.length ? values[0] : void 0,
  static: true
});
var changeFilter2 = Facet2.define();
var transactionFilter2 = Facet2.define();
var transactionExtender2 = Facet2.define();
var readOnly2 = Facet2.define({
  combine: (values) => values.length ? values[0] : false
});
var Annotation2 = class {
  /**
  @internal
  */
  constructor(type, value) {
  ...
  */
  static define() {
    return new AnnotationType2();
  }
};
var AnnotationType2 = class {
  /**
  Create an instance of this annotation.
  */
  of(value) {
    return new Annotation2(this, value);
  }
};
var StateEffectType2 = class {
  /**
  @internal
  */
  constructor(map) {
...
var StateEffect2 = class _StateEffect {
  ...
  */
  static define(spec = {}) {
    return new StateEffectType2(spec.map || ((v) => v));
  }
  /**
  Map an array of effects through a change set.
  */
...
var Transaction2 = class _Transaction {
  constructor(startState, changes, selection, effects, annotations, scrollIntoView3) {
    ...
    this._state = null;
    if (selection)
      checkSelection2(selection, changes.newLength);
    if (!annotations.some((a) => a.type == _Transaction.time))
      this.annotations = annotations.concat(_Transaction.time.of(Date.now()));
  }
  /**
...
Transaction2.time = Annotation2.define();
Transaction2.userEvent = Annotation2.define();
Transaction2.addToHistory = Annotation2.define();
Transaction2.remote = Annotation2.define();
function joinRanges2(a, b) {
  let result = [];
  for (let iA = 0, iB = 0; ; ) {
    let from, to;
    if (iA < a.length && (iB == b.length || b[iB] >= a[iA])) {
...
function mergeTransaction2(a, b, sequential) {
  var _a2;
  let mapForA, mapForB, changes;
  if (sequential) {
    mapForA = b.changes;
    mapForB = ChangeSet2.empty(b.changes.length);
    changes = a.changes.compose(b.changes);
  } else {
    mapForA = b.changes.map(a.changes);
    mapForB = a.changes.mapDesc(b.changes, true);
...
function resolveTransaction2(state, specs, filter) {
  let s = resolveTransactionInner2(state, specs.length ? specs[0] : {}, state.doc.length);
  if (specs.length && specs[0].filter === false)
    filter = false;
  for (let i = 1; i < specs.length; i++) {
    if (specs[i].filter === false)
      filter = false;
    let seq = !!specs[i].sequential;
    s = mergeTransaction2(s, resolveTransactionInner2(state, specs[i], seq ? s.changes.newLength : state.doc.length), seq);
  }
  let tr = Transaction2.create(state, s.changes, s.selection, s.effects, s.annotations, s.scrollIntoView);
  return extendTransaction2(filter ? filterTransaction2(tr) : tr);
}
function filterTransaction2(tr) {
  let state = tr.startState;
  let result = true;
  for (let filter of state.facet(changeFilter2)) {
    let value = filter(tr);
    if (value === false) {
      result = false;
      break;
    ...
    if (Array.isArray(value))
      result = result === true ? value : joinRanges2(result, value);
  }
  if (result !== true) {
    let changes, back;
    if (result === false) {
      back = tr.changes.invertedDesc;
      changes = ChangeSet2.empty(state.doc.length);
    } else {
      let filtered = tr.changes.filter(result);
      changes = filtered.changes;
      back = filtered.filtered.mapDesc(filtered.changes).invertedDesc;
    }
    tr = Transaction2.create(state, changes, tr.selection && tr.selection.map(back), StateEffect2.mapEffects(tr.effects, back), tr.annotations, tr.scrollIntoView);
  }
  let filters = state.facet(transactionFilter2);
  for (let i = filters.length - 1; i >= 0; i--) {
    let filtered = filters[i](tr);
...
function extendTransaction2(tr) {
  let state = tr.startState, extenders = state.facet(transactionExtender2), spec = tr;
  for (let i = extenders.length - 1; i >= 0; i--) {
    let extension = extenders[i](tr);
    if (extension && Object.keys(extension).length)
      spec = mergeTransaction2(spec, resolveTransactionInner2(state, extension, tr.changes.newLength), true);
  }
...
var CharCategory2 = function(CharCategory3) {
  CharCategory3[CharCategory3["Word"] = 0] = "Word";
  CharCategory3[CharCategory3["Space"] = 1] = "Space";
  CharCategory3[CharCategory3["Other"] = 2] = "Other";
  return CharCategory3;
}(CharCategory2 || (CharCategory2 = {}));
var nonASCIISingleCaseWordChar2 = /[\u00df\u0587\u0590-\u05f4\u0600-\u06ff\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc\uac00-\ud7af]/;
var wordChar2;
try {
  wordChar2 = new RegExp("[\\p{Alphabetic}\\p{Number}_]", "u");
} catch (_) {
}
function hasWordChar2(str) {
  if (wordChar2)
    return wordChar2.test(str);
  for (let i = 0; i < str.length; i++) {
    let ch = str[i];
    if (/\w/.test(ch) || ch > "" && (ch.toUpperCase() != ch.toLowerCase() || nonASCIISingleCaseWordChar2.test(ch)))
      return true;
  }
  return false;
}
function makeCategorizer2(wordChars) {
  return (char) => {
    if (!/\S/.test(char))
      return CharCategory2.Space;
    if (hasWordChar2(char))
      return CharCategory2.Word;
    for (let i = 0; i < wordChars.length; i++)
      if (char.indexOf(wordChars[i]) > -1)
        return CharCategory2.Word;
    return CharCategory2.Other;
  };
}
var EditorState2 = class _EditorState {
  ...
  field(field, require2 = true) {
    ...
    ensureAddr2(this, addr);
    return getAddr2(this, addr);
  }
  /**
  Create a [transaction](https://codemirror.net/6/docs/ref/#state.Transaction) that updates this
  state. Any number of [transaction specs](https://codemirror.net/6/docs/ref/#state.TransactionSpec)
  ...
  */
  update(...specs) {
    return resolveTransaction2(this, specs, true);
  }
  /**
  @internal
  */
  applyTransaction(tr) {
    let conf = this.config, { base: base2, compartments } = conf;
    for (let effect of tr.effects) {
      if (effect.is(Compartment2.reconfigure)) {
        if (conf) {
          compartments = /* @__PURE__ */ new Map();
          conf.compartments.forEach((val, key) => compartments.set(key, val));
          conf = null;
    ...
    let selection = tr.startState.facet(allowMultipleSelections2) ? tr.newSelection : tr.newSelection.asSingle();
    new _EditorState(conf, tr.newDoc, selection, startValues, (state, slot) => slot.update(state, tr), tr);
  }
  /**
  Create a [transaction spec](https://codemirror.net/6/docs/ref/#state.TransactionSpec) that
  ...
  */
  replaceSelection(text) {
    ...
    return this.changeByRange((range) => ({
      changes: { from: range.from, to: range.to, insert: text },
      range: EditorSelection2.cursor(range.from + text.length)
    }));
  }
  /**
  Create a set of changes and a new selection by running the given
  ...
  */
  changeByRange(f) {
    ...
    return {
      changes,
      selection: EditorSelection2.create(ranges, sel.mainIndex),
      effects
    };
  }
  /**
  ...
  */
  changes(spec = []) {
    if (spec instanceof ChangeSet2)
      return spec;
    return ChangeSet2.of(spec, this.doc.length, this.facet(_EditorState.lineSeparator));
  }
  /**
  Using the state's [line
  separator](https://codemirror.net/6/docs/ref/#state.EditorState^lineSeparator), create a
  ...
  */
  toText(string3) {
    return Text2.of(string3.split(this.facet(_EditorState.lineSeparator) || DefaultSplit2));
  }
  /**
  Return the given range of the document as a string.
  */
  ...
  */
  static fromJSON(json, config2 = {}, fields) {
    ...
    return _EditorState.create({
      doc: json.doc,
      selection: EditorSelection2.fromJSON(json.selection),
      extensions: config2.extensions ? fieldInit.concat([config2.extensions]) : fieldInit
    });
  }
  /**
  ...
  */
  static create(config2 = {}) {
    let configuration = Configuration2.resolve(config2.extensions || [], /* @__PURE__ */ new Map());
    let doc2 = config2.doc instanceof Text2 ? config2.doc : Text2.of((config2.doc || "").split(configuration.staticFacet(_EditorState.lineSeparator) || DefaultSplit2));
    let selection = !config2.selection ? EditorSelection2.single(0) : config2.selection instanceof EditorSelection2 ? config2.selection : EditorSelection2.single(config2.selection.anchor, config2.selection.head);
    checkSelection2(selection, doc2.length);
    if (!configuration.staticFacet(allowMultipleSelections2))
      selection = selection.asSingle();
    return new _EditorState(configuration, doc2, selection, configuration.dynamicSlots.map(() => null), (state, slot) => slot.create(state), null);
  }
  /**
  ...
  */
  languageDataAt(name3, pos, side = -1) {
    let values = [];
    for (let provider of this.facet(languageData2)) {
      for (let result of provider(this, pos, side)) {
        if (Object.prototype.hasOwnProperty.call(result, name3))
          values.push(result[name3]);
      }
  ...
  */
  charCategorizer(at) {
    return makeCategorizer2(this.languageDataAt("wordChars", at).join(""));
  }
  /**
  Find the word at the given position, meaning the range
  containing all [word](https://codemirror.net/6/docs/ref/#state.CharCategory.Word) characters
  ...
  */
  wordAt(pos) {
    ...
    while (start > 0) {
      ...
      if (cat(text.slice(prev, start)) != CharCategory2.Word)
        break;
      start = prev;
    }
    while (end < length) {
      let next = findClusterBreak2(text, end);
      if (cat(text.slice(end, next)) != CharCategory2.Word)
        break;
      end = next;
    }
    return start == end ? null : EditorSelection2.range(start + from, end + from);
  }
};
EditorState2.allowMultipleSelections = allowMultipleSelections2;
EditorState2.tabSize = Facet2.define({
  combine: (values) => values.length ? values[0] : 4
});
EditorState2.lineSeparator = lineSeparator2;
EditorState2.readOnly = readOnly2;
EditorState2.phrases = Facet2.define({
  compare(a, b) {
    let kA = Object.keys(a), kB = Object.keys(b);
    return kA.length == kB.length && kA.every((k) => a[k] == b[k]);
  }
});
EditorState2.languageData = languageData2;
EditorState2.changeFilter = changeFilter2;
EditorState2.transactionFilter = transactionFilter2;
EditorState2.transactionExtender = transactionExtender2;
Compartment2.reconfigure = StateEffect2.define();
function combineConfig2(configs, defaults2, combine = {}) {
  let result = {};
  for (let config2 of configs)
    for (let key of Object.keys(config2)) {
      let value = config2[key], current = result[key];
...
RangeValue2.prototype.startSide = RangeValue2.prototype.endSide = 0;
RangeValue2.prototype.point = false;
RangeValue2.prototype.mapMode = MapMode2.TrackDel;
...
var RangeSet2 = class _RangeSet {
  ...
  */
  iter(from = 0) {
    return HeapCursor2.from([this]).goto(from);
  }
  /**
  @internal
  */
  ...
  */
  static iter(sets, from = 0) {
    return HeapCursor2.from(sets).goto(from);
  }
  /**
  Iterate over two groups of sets, calling methods on `comparator`
  to notify it of possible differences.
  */
  static compare(oldSets, newSets, textDiff, comparator, minPointSize = -1) {
    let a = oldSets.filter((set) => set.maxPoint > 0 || !set.isEmpty && set.maxPoint >= minPointSize);
    let b = newSets.filter((set) => set.maxPoint > 0 || !set.isEmpty && set.maxPoint >= minPointSize);
    let sharedChunks = findSharedChunks2(a, b, textDiff);
    let sideA = new SpanCursor2(a, sharedChunks, minPointSize);
    let sideB = new SpanCursor2(b, sharedChunks, minPointSize);
  ...
  */
  static eq(oldSets, newSets, from = 0, to) {
    ...
    if (!a.length)
      return true;
    let sharedChunks = findSharedChunks2(a, b);
    let sideA = new SpanCursor2(a, sharedChunks, 0).goto(from), sideB = new SpanCursor2(b, sharedChunks, 0).goto(from);
    for (; ; ) {
      if (sideA.to != sideB.to || !sameValues2(sideA.active, sideB.active) || sideA.point && (!sideB.point || !sideA.point.eq(sideB.point)))
        return false;
      if (sideA.to > to)
        return true;
      sideA.next();
  ...
  */
  static spans(sets, from, to, iterator, minPointSize = -1) {
    let cursor = new SpanCursor2(sets, null, minPointSize).goto(from), pos = from;
    let openRanges = cursor.openStart;
    for (; ; ) {
      let curTo = Math.min(cursor.to, to);
      if (cursor.point) {
  ...
  */
  static of(ranges, sort = false) {
    ...
    for (let range of ranges instanceof Range4 ? [ranges] : sort ? lazySort2(ranges) : ranges)
      build.add(range.from, range.to, range.value);
    return build.finish();
  }
  /**
...
RangeSet2.empty = new RangeSet2([], [], null, -1);
function lazySort2(ranges) {
  if (ranges.length > 1)
    for (let prev = ranges[0], i = 1; i < ranges.length; i++) {
      let cur2 = ranges[i];
      if (cmpRange2(prev, cur2) > 0)
        return ranges.slice().sort(cmpRange2);
      prev = cur2;
    }
  return ranges;
}
RangeSet2.empty.nextLayer = RangeSet2.empty;
var RangeSetBuilder2 = class _RangeSetBuilder {
  finishChunk(newArrays) {
    this.chunks.push(new Chunk2(this.from, this.to, this.value, this.maxPoint));
    this.chunkPos.push(this.chunkStart);
    this.chunkStart = -1;
    this.setMaxPoint = Math.max(this.setMaxPoint, this.maxPoint);
    this.maxPoint = -1;
  ...
  */
  finish() {
    return this.finishInner(RangeSet2.empty);
  }
  /**
  @internal
  */
  finishInner(next) {
    ...
    if (this.chunks.length == 0)
      return next;
    let result = RangeSet2.create(this.chunkPos, this.chunks, this.nextLayer ? this.nextLayer.finishInner(next) : next, this.setMaxPoint);
    this.from = null;
    return result;
  }
};
function findSharedChunks2(a, b, textDiff) {
  let inA = /* @__PURE__ */ new Map();
  for (let set of a)
    for (let i = 0; i < set.chunk.length; i++)
      if (set.chunk[i].maxPoint <= 0)
...
var HeapCursor2 = class _HeapCursor {
  constructor(heap) {
    this.heap = heap;
  }
  static from(sets, skip = null, minPoint = -1) {
    ...
    for (let i = 0; i < sets.length; i++) {
      for (let cur2 = sets[i]; !cur2.isEmpty; cur2 = cur2.nextLayer) {
        if (cur2.maxPoint >= minPoint)
          heap.push(new LayerCursor2(cur2, skip, minPoint, i));
      }
    }
    return heap.length == 1 ? heap[0] : new _HeapCursor(heap);
  }
  ...
  next() {
    if (this.heap.length == 0) {
      ...
      if (top.value)
        top.next();
      heapBubble2(this.heap, 0);
    }
  }
};
function heapBubble2(heap, index) {
  for (let cur2 = heap[index]; ; ) {
    let childIndex = (index << 1) + 1;
    if (childIndex >= heap.length)
      break;
...
var SpanCursor2 = class {
  constructor(sets, skip, minPoint) {
    ...
    this.endSide = 0;
    this.openStart = -1;
    this.cursor = HeapCursor2.from(sets, skip, minPoint);
  }
  goto(pos, side = -1e9) {
    this.cursor.goto(pos, side);
    this.active.length = this.activeTo.length = this.activeRank.length = 0;
  ...
  removeActive(index) {
    ...
    this.minActive = findMinIndex2(this.active, this.activeTo);
  }
  addActive(trackOpen) {
    let i = 0, { value, to, rank } = this.cursor;
    while (i < this.activeRank.length && (rank - this.activeRank[i] || to - this.activeTo[i]) > 0)
...
function compare2(a, startA, b, startB, length, comparator) {
  ...
  for (; ; ) {
    ...
    let end = diff < 0 ? a.to + dPos : b.to, clipEnd = Math.min(end, endB);
    if (a.point || b.point) {
      if (!(a.point && b.point && (a.point == b.point || a.point.eq(b.point)) && sameValues2(a.activeForPoint(a.to), b.activeForPoint(b.to))))
        comparator.comparePoint(pos, clipEnd, a.point, b.point);
    } else {
      if (clipEnd > pos && !sameValues2(a.active, b.active))
        comparator.compareRange(pos, clipEnd, a.active, b.active);
    }
    if (end > endB)
      break;
...
function findMinIndex2(value, array) {
  let found = -1, foundPos = 1e9;
  for (let i = 0; i < array.length; i++)
    if ((array[i] - foundPos || value[i].endSide - value[found].endSide) < 0) {
      found = i;
...
function replaceRange(parent, fromI, fromOff, toI, toOff, insert3, breakAtStart, openStart, openEnd) {
  ...
  while (fromI < toI && insert3.length) {
    if (children[toI - 1].become(insert3[insert3.length - 1])) {
      toI--;
      insert3.pop();
      openEnd = insert3.length ? 0 : openStart;
    } else if (children[fromI].become(insert3[0])) {
      fromI++;
      insert3.shift();
  ...
  if (!insert3.length && fromI && toI < children.length && !children[fromI - 1].breakAfter && children[toI].merge(0, 0, children[fromI - 1], false, openStart, openEnd))
    fromI--;
...
var Decoration = class extends RangeValue2 {
...
Decoration.none = RangeSet2.empty;
...
LineDecoration.prototype.mapMode = MapMode2.TrackBefore;
...
var PointDecoration = class _PointDecoration extends Decoration {
  constructor(spec, startSide, endSide, block, widget, isReplace) {
    ...
    this.mapMode = !block ? MapMode2.TrackDel : startSide <= 0 ? MapMode2.TrackBefore : MapMode2.TrackAfter;
  }
...
var ContentBuilder = class _ContentBuilder {
  ...
  static build(text, from, to, decorations2, dynamicDecorationMap) {
    ...
    builder.openEnd = RangeSet2.spans(decorations2, from, to, builder);
...
function moveVisually(line, order, dir, start, forward) {
  ...
  if (nextSpan && nextIndex == spanEnd && nextSpan.level + (forward ? 0 : 1) < span.level)
    return EditorSelection2.cursor(nextSpan.side(!forward, dir) + line.from, nextSpan.forward(forward, dir) ? 1 : -1, nextSpan.level);
  return EditorSelection2.cursor(nextIndex + line.from, span.forward(forward, dir) ? -1 : 1, span.level);
}
...
var clickAddsSelectionRange = Facet2.define();
var dragMovesSelection$1 = Facet2.define();
var mouseSelectionStyle = Facet2.define();
var exceptionSink = Facet2.define();
var updateListener = Facet2.define();
var inputHandler = Facet2.define();
var focusChangeEffect = Facet2.define();
var clipboardInputFilter = Facet2.define();
var clipboardOutputFilter = Facet2.define();
var perLineTextDirection = Facet2.define({
  combine: (values) => values.some((x) => x)
});
var nativeSelectionHidden = Facet2.define({
  combine: (values) => values.some((x) => x)
});
var scrollHandler = Facet2.define();
var ScrollTarget = class _ScrollTarget {
  ...
  clip(state) {
    return this.range.to <= state.doc.length ? this : new _ScrollTarget(EditorSelection2.cursor(state.doc.length), this.y, this.x, this.yMargin, this.xMargin, this.isSnapshot);
  }
};
var scrollIntoView = StateEffect2.define({ map: (t3, ch) => t3.map(ch) });
var setEditContextFormatting = StateEffect2.define();
...
var editable = Facet2.define({ combine: (values) => values.length ? values[0] : true });
var nextPluginID = 0;
var viewPlugin = Facet2.define();
...
var editorAttributes = Facet2.define();
var contentAttributes = Facet2.define();
var decorations = Facet2.define();
var outerDecorations = Facet2.define();
var atomicRanges = Facet2.define();
var bidiIsolatedRanges = Facet2.define();
function getIsolatedRanges(view, line) {
  ...
  let result = [];
  RangeSet2.spans(sets, line.from, line.to, {
    point() {
    },
...
var scrollMargins = Facet2.define();
...
var ViewUpdate = class _ViewUpdate {
  constructor(view, state, transactions) {
    ...
    this.changes = ChangeSet2.empty(this.startState.doc.length);
...
var DocView = class extends ContentView {
  ...
  updateChildren(changes, oldLength, composition) {
    ...
    for (let i = ranges.length - 1; ; i--) {
      ...
      if (!next)
        break;
      let { fromA, toA, fromB, toB } = next, content3, breakAtStart, openStart, openEnd;
  ...
  updateDeco() {
    ...
    if (outerDeco.length) {
      ...
      allDeco.push(RangeSet2.join(outerDeco));
    }
    this.decorations = [
...
function findChangedDeco(a, b, diff) {
  ...
  RangeSet2.compare(a, b, diff, comp);
  return comp.changes;
}
...
function groupAt(state, pos, bias = 1) {
  ...
  if (line.length == 0)
    return EditorSelection2.cursor(pos);
  if (linePos == 0)
    bias = 1;
  ...
  return EditorSelection2.range(from + line.from, to + line.from);
}
...
function moveToLineBoundary(view, start, forward, includeWrap) {
  ...
  if (coords) {
    ...
    if (pos != null)
      return EditorSelection2.cursor(pos, forward ? -1 : 1);
  }
  return EditorSelection2.cursor(forward ? line.to : line.from, forward ? -1 : 1);
}
...
function moveVertically(view, start, forward, distance) {
  ...
  if (startPos == (forward ? view.state.doc.length : 0))
    return EditorSelection2.cursor(startPos, start.assoc);
  ...
  for (let extra = 0; ; extra += 10) {
    ...
    if (curY < rect.top || curY > rect.bottom || (dir < 0 ? pos < startPos : pos > startPos)) {
      ...
      return EditorSelection2.cursor(pos, assoc, void 0, goal);
    }
  }
}
...
function skipAtoms(view, oldPos, pos) {
  ...
  return newPos == pos.from ? pos : EditorSelection2.cursor(newPos, newPos < pos.from ? 1 : -1);
}
...
var DOMReader = class {
  constructor(points, state) {
    this.points = points;
    this.text = "";
    this.lineSeparator = state.facet(EditorState2.lineSeparator);
  }
  append(text) {
    this.text += text;
  }
...
var DOMChange = class {
  constructor(view, start, end, typeOver) {
    ...
    if (view.state.readOnly && start > -1) {
      ...
      this.newSel = EditorSelection2.single(anchor, head);
    }
  }
};
function applyDOMChange(view, domChange) {
  ...
  if (!change && domChange.typeOver && !sel.empty && newSel && newSel.main.empty) {
    ...
    if (newSel && change.insert.length == 2)
      newSel = EditorSelection2.single(newSel.main.anchor - 1, newSel.main.head - 1);
    ...
    if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 && change.to <= sel.to && change.to >= sel.to - 10) {
      ...
      tr = startState.changeByRange((range) => {
        ...
        return {
          ...
          range: !mainSel ? range.map(rangeChanges) : EditorSelection2.range(Math.max(0, mainSel.anchor + selOff), Math.max(0, mainSel.head + selOff))
        };
      });
    } else {
      tr = {
...
function selectionFromPoints(points, base2) {
  ...
  return anchor > -1 && head > -1 ? EditorSelection2.single(anchor + base2, head + base2) : null;
}
...
var MouseSelection = class {
  constructor(view, startEvent, style, mustSelect) {
    ...
    this.multiple = view.state.facet(EditorState2.allowMultipleSelections) && addsSelectionRange(view, startEvent);
  ...
  skipAtoms(sel) {
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
      ...
      if (range.empty) {
        ...
        if (pos != range.from)
          updated = EditorSelection2.cursor(pos, -1);
      } else {
        let from = skipAtomicRanges(this.atoms, range.from, -1);
        let to = skipAtomicRanges(this.atoms, range.to, 1);
        if (from != range.from || to != range.to)
          updated = EditorSelection2.range(range.from == range.anchor ? from : to, range.from == range.head ? from : to);
      }
      if (updated) {
        if (!ranges)
    ...
    return ranges ? EditorSelection2.create(ranges, sel.mainIndex) : sel;
  }
  select(event) {
  ...
  if (linewise) {
    ...
    changes = state.changeByRange((range) => {
      ...
      return {
        ...
        range: EditorSelection2.cursor(range.from + insert3.length)
      };
    });
  } else if (byLine) {
    changes = state.changeByRange((range) => {
      ...
      return {
        ...
        range: EditorSelection2.cursor(range.from + line.length)
      };
    });
  } else {
...
function rangeForClick(view, pos, bias, type) {
  if (type == 1) {
    return EditorSelection2.cursor(pos, bias);
  } else if (type == 2) {
  ...
  return {
    ...
    get(event2, extend3, multiple) {
      ...
      if (start.pos != cur2.pos && !extend3) {
        ...
        range = from < range.from ? EditorSelection2.range(from, to) : EditorSelection2.range(to, from);
      }
      if (extend3)
        ...
        return EditorSelection2.create([range]);
    }
  };
}
function removeRangeAround(sel, pos) {
  for (let i = 0; i < sel.ranges.length; i++) {
    ...
    if (from <= pos && to >= pos)
      return EditorSelection2.create(sel.ranges.slice(0, i).concat(sel.ranges.slice(i + 1)), sel.mainIndex == i ? 0 : sel.mainIndex - (sel.mainIndex > i ? 1 : 0));
  }
  return null;
}
  ...
  if (event.target.draggable) {
    ...
    if (cView && cView.isWidget) {
      ...
      if (from >= range.to || to <= range.from)
        range = EditorSelection2.range(from, to);
    }
  }
  let { inputState } = view;
...
var isFocusChange = Annotation2.define();
...
var NodeBuilder = class _NodeBuilder {
  ...
  static build(oracle, decorations2, from, to) {
    ...
    RangeSet2.spans(decorations2, from, to, builder, 0);
    return builder.finish(from);
  }
};
function heightRelevantDecoChanges(a, b, diff) {
  let comp = new DecorationComparator2();
  RangeSet2.compare(a, b, diff, comp, 0);
  return comp.changes;
}
...
var ViewState = class {
  ...
  update(update, scrollTarget = null) {
    ...
    let heightChanges = ChangedRange.extendWithRanges(contentChanges, heightRelevantDecoChanges(prevDeco, this.stateDeco, update ? update.changes : ChangeSet2.empty(this.state.doc.length)));
    ...
    let addGap = (from, to, line, structure) => {
      ...
      if (!gap) {
        if (to < line.to && mayMeasure && wrapping && mayMeasure.visibleRanges.some((r) => r.from <= to && r.to >= to)) {
          let lineStart = mayMeasure.moveToLineBoundary(EditorSelection2.cursor(to), false, true).head;
          if (lineStart > from)
            to = lineStart;
        }
  ...
  computeVisibleRanges() {
    ...
    let ranges = [];
    RangeSet2.spans(deco, this.viewport.from, this.viewport.to, {
      span(from, to) {
        ranges.push({ from, to });
      },
      point() {
...
function lineStructure(from, to, stateDeco) {
  ...
  RangeSet2.spans(stateDeco, from, to, {
    span() {
    },
    point(from2, to2) {
      if (from2 > pos) {
...
var theme = Facet2.define({ combine: (strs) => strs.join(" ") });
var darkTheme = Facet2.define({ combine: (values) => values.indexOf(true) > -1 });
...
var EditContextManager = class {
  constructor(view) {
    ...
    this.handlers.textupdate = (e) => {
      ...
      if (!view.state.readOnly)
        applyDOMChangeInner(view, change, EditorSelection2.single(this.toEditorPos(e.selectionStart), this.toEditorPos(e.selectionEnd)));
...
var EditorView = class _EditorView {
  ...
  */
  constructor(config2 = {}) {
    ...
    this.viewState = new ViewState(config2.state || EditorState2.create(config2));
  ...
  */
  update(transactions) {
    ...
    if (state.facet(EditorState2.phrases) != this.state.facet(EditorState2.phrases))
    ...
    try {
      ...
      for (let tr of transactions) {
        ...
        if (tr.scrollIntoView) {
          ...
          scrollTarget = new ScrollTarget(main.empty ? main : EditorSelection2.cursor(main.head, main.head > main.anchor ? -1 : 1));
        }
  ...
  */
  visualLineSide(line, end) {
    ...
    return EditorSelection2.cursor(span.side(end, dir) + line.from, span.forward(!end, dir) ? 1 : -1);
  }
  /**
  ...
  */
  static scrollIntoView(pos, options = {}) {
    return scrollIntoView.of(new ScrollTarget(typeof pos == "number" ? EditorSelection2.cursor(pos) : pos, options.y, options.x, options.yMargin, options.xMargin));
  }
  /**
  ...
  */
  scrollSnapshot() {
    ...
    return scrollIntoView.of(new ScrollTarget(EditorSelection2.cursor(ref.from), "start", "start", ref.top - scrollTop, scrollLeft, true));
  }
  /**
  ...
  */
  static baseTheme(spec) {
    return Prec2.lowest(styleModule.of(buildTheme("." + baseThemeID, spec, lightDarkIDs)));
  }
  /**
...
EditorView.cspNonce = Facet2.define({ combine: (values) => values.length ? values[0] : "" });
EditorView.contentAttributes = contentAttributes;
EditorView.editorAttributes = editorAttributes;
EditorView.lineWrapping = EditorView.contentAttributes.of({ "class": "cm-lineWrapping" });
EditorView.announce = StateEffect2.define();
...
var handleKeyEvents = Prec2.default(EditorView.domEventHandlers({
  keydown(event, view) {
    return runHandlers(getKeymap(view.state), event, view, "editor");
  }
}));
var keymap = Facet2.define({ enables: handleKeyEvents });
...
var selectionConfig = Facet2.define({
  combine(configs) {
  ...
  markers(view) {
    ...
    for (let r of state.selection.ranges) {
      ...
      if (r.empty ? !prim || CanHidePrimary : conf.drawRangeCursor) {
        ...
        let cursor = r.empty ? r : EditorSelection2.cursor(r.head, r.head > r.anchor ? -1 : 1);
...
var hideNativeSelection = Prec2.highest(EditorView.theme(themeSpec));
var setDropCursorPos = StateEffect2.define({
  map(pos, mapping) {
    return pos == null ? null : mapping.mapPos(pos);
  }
});
var dropCursorPos = StateField2.define({
  create() {
    return null;
  },
  update(pos, tr) {
...
var MatchDecorator = class {
  ...
  */
  createDeco(view) {
    let build = new RangeSetBuilder2(), add = build.add.bind(build);
...
var specialCharConfig = Facet2.define({
  combine(configs) {
    let config2 = combineConfig2(configs, {
      render: null,
...
var tooltipConfig = Facet2.define({
  combine: (values) => {
    var _a2, _b, _c;
    return {
...
var showTooltip = Facet2.define({
  enables: [tooltipPlugin, baseTheme]
});
var showHoverTooltip = Facet2.define({
...
var closeHoverTooltipEffect = StateEffect2.define();
var closeHoverTooltips = closeHoverTooltipEffect.of(null);
var panelConfig = Facet2.define({
  combine(configs) {
...
var showPanel = Facet2.define({
  enables: panelPlugin
});
var GutterMarker = class extends RangeValue2 {
  /**
  @internal
  */
  compare(other) {
...
GutterMarker.prototype.mapMode = MapMode2.TrackBefore;
GutterMarker.prototype.startSide = GutterMarker.prototype.endSide = -1;
GutterMarker.prototype.point = true;
var gutterLineClass = Facet2.define();
var gutterWidgetClass = Facet2.define();
var activeGutters = Facet2.define();
var unfixGutters = Facet2.define({
  combine: (values) => values.some((x) => x)
});
var gutterView = ViewPlugin.fromClass(class {
  ...
  syncGutters(detach) {
    ...
    if (detach)
      this.dom.remove();
    let lineClasses = RangeSet2.iter(this.view.state.facet(gutterLineClass), this.view.viewport.from);
    let classSet = [];
  ...
  updateGutters(update) {
    ...
    let change = update.docChanged || update.heightChanged || update.viewportChanged || !RangeSet2.eq(update.startState.facet(gutterLineClass), update.state.facet(gutterLineClass), update.view.viewport.from, update.view.viewport.to);
    if (prev == cur2) {
      for (let gutter2 of this.gutters)
...
var UpdateContext = class {
  constructor(gutter2, viewport, height) {
    ...
    this.height = height;
    this.i = 0;
    this.cursor = RangeSet2.iter(gutter2.markers, viewport.from);
  }
...
var SingleGutterView = class {
  ...
  update(update) {
    ...
    return !RangeSet2.eq(this.markers, prevMarkers, vp.from, vp.to) || (this.config.lineMarkerChange ? this.config.lineMarkerChange(update) : false);
  }
  destroy() {
...
var lineNumberMarkers = Facet2.define();
var lineNumberWidgetMarker = Facet2.define();
var lineNumberConfig = Facet2.define({
  combine(values) {
    return combineConfig2(values, { formatNumber: String, domEventHandlers: {} }, {
...
var Modifier2 = class _Modifier {
  ...
  static get(base2, mods) {
    if (!mods.length)
      return base2;
    let exists = mods[0].instances.find((t3) => t3.base == base2 && sameArray4(mods, t3.modified));
    if (exists)
      return exists;
...
var inputHandler2 = EditorView.inputHandler.of((view, from, to, insert3) => {
...
function pathFor(read, member, name3) {
  var _a2;
  let path = [];
  for (; ; ) {
    ...
    if ((obj === null || obj === void 0 ? void 0 : obj.name) == "VariableName") {
      path.push(read(obj));
      return { path: path.reverse(), name: name3 };
...
function enumeratePropertyCompletions(obj, top) {
  ...
  for (let depth = 0; ; depth++) {
    for (let name3 of (Object.getOwnPropertyNames || Object.keys)(obj)) {
      if (!/^[a-zA-Z_$\xaa-\uffdc][\w$\xaa-\uffdc]*$/.test(name3) || seen.has(name3))
        continue;
      seen.add(name3);
      let value;
      try {
      ...
      options.push({
        label: name3,
        type: typeof value == "function" ? /^[A-Z]/.test(name3) ? "class" : top ? "function" : "method" : top ? "variable" : "property",
        boost: -depth
      });
    }
...
var kwCompletion = (name3) => ({ label: name3, type: "keyword" });
...
var autoCloseTags = EditorView.inputHandler.of((view, from, to, text, defaultInsert) => {
  ...
  let closeTags = state.changeByRange((range) => {
    var _a2;
    let { head } = range, around = syntaxTree(state).resolveInner(head - 1, -1), name3;
    ...
    if (state.doc.sliceString(head - 1, head) != text || around.name == "JSXAttributeValue" && around.to > head) ;
      ...
      if (base3 && empty.from == head - 2 && ((name3 = elementName(state.doc, base3.firstChild, head)) || ((_a2 = base3.firstChild) === null || _a2 === void 0 ? void 0 : _a2.name) == "JSXFragmentTag")) {
        let insert3 = `${name3}>`;
        return { range: EditorSelection.cursor(head + insert3.length, -1), changes: { from: head, insert: insert3 } };
      }
    } else if (text == ">") {
      let openTag = findOpenTag(around);
      if (openTag && openTag.name == "JSXOpenTag" && !/^\/?>|^<\//.test(state.doc.sliceString(head, head + 2)) && (name3 = elementName(state.doc, openTag, head)))
        return { range, changes: { from: head, insert: `</${name3}>` } };
    }
    return { range };
  });
...
function esLint(eslint, config2) {
  if (!config2) {
    ...
    eslint.getRules().forEach((desc, name3) => {
      if (desc.meta.docs.recommended)
        config2.rules[name3] = 2;
    });
  }
  return (view) => {
